...
- Concretely showed that an equivalence between Aeneas generated Rust code and Lean specifications for SHA1 and AES can be verified, up to some `sorry`s that we justify for specific reasons.
...
- For the remaining `sorry`s, we've provided inline documentation on why it is either insignificant or can be elided without impacting the trust of the equivalence. We put them each in several cagetories:
    - Bounds proofs: Hypotheses of the form `i < n` or `k < USize.size` are often needed by Aeneas to justify Rust semantics or loop invariants. Yet due to some `simp` limitations, are annoying to deal with without better automation -- the ones that are obviously true are `sorry`ed. For the ones that are not obviously true, we provide a complete proof.
    - Arithmetic hammer needed: Better automation is needed to deal with computations that are equivalent, but need some kind of transformation, e.g. loop unrolling, optimizations that reorder operations, etc.
    - No Aeneas implementation: As Aeneas is not fully mature, some definitions have no implementation. Thus we cannot reason about them.
    - Similar proof completed elsewhere: These are  statements which have correspondingly similar statements for which we've already proved, and due to the complexity/annoyance of the proof, have decided to save space by adding a `sorry` here.
    - True but annoying: These are statements are obviously true, but for any number of reasons (not enough Mathlib/Aeneas support, heavily repetitive proofs) are too much effort to be worth filling in.
...
- `Structured`: This representation is a refactored version of the Lean specification, and is slightly more structured so as to closer reflect the Aeneas structure. Aeneas generated code has a very rigid structure (e.g. every for loop will be factored out into a `*_loop` function).
...
- Bounds-checking: Aeneas faithfully models Rust integer types, like `u32`, whereas Lean often uses `Nat`. Thus, a significant amount of effort has to be taken to show that the Rust model will not "overflow," e.g. when adding two `u32`s or pushing to a vector. Most of these bounds are clearly trivial, and so we've justified leaving some as `sorry`s for now. However, they are a clear source of bugs, where, if not modeled correctly, we may find subtle missed assumptions.
    - One example of the subtle missed assumptions is translating the statement: `let zero_padding_length = (56 - len % 64) % 64;`. The corresponding correct Lean statement is `let zeroPaddingLength := (64 - len % 64 + 56) % 64` NOT `let zeroPaddingLength := (56 - len % 64) % 64`. When `len % 64` is greater than `56`, Rust substraction will wrap around `usize::MAX`, and maintain the expected modulus. In Lean, subtraction is saturating, and will clip to `0 : Nat`, thus losing the expected modulus. 